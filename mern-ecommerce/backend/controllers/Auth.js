...
exports.login = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Some error occured while logging in, please try again later' });
  }
};
...
exports.resendOtp = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Some error occured while resending otp, please try again later' });
    console.log(error);
  }
};
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Error occured while resetting the password, please try again later' });
  }
};
...
exports.googleAuth = async (req, res) => {
  try {
    const redirectUri =
      process.env.GOOGLE_REDIRECT_URI || `${process.env.ORIGIN}/api/auth/google/callback`;
    const authUrl =
...
exports.googleCallback = async (req, res) => {
  try {
    ...
    if (!userInfoResult.success) {
      return res.status(400).json({ success: false, message: userInfoResult.error });
    }

    const { id, email, name, picture } = userInfoResult.data;
