...
async function verifyModels() {
  ...
  if (sampleProduct) {
    ...
    console.log(`   - averageRating field: ${typeof sampleProduct.averageRating === 'number' ? '✅' : '❌'}`);
    console.log(`   - numReviews field: ${typeof sampleProduct.numReviews === 'number' ? '✅' : '❌'}`);
    console.log(`   - Sample values: Rating ${sampleProduct.averageRating}, Reviews ${sampleProduct.numReviews}`);
  } else {
  ...
  if (sampleReview) {
    ...
    console.log(`   - Sample review: ${sampleReview.rating}⭐ "${sampleReview.comment.substring(0, 50)}..."`);
  } else {
...
async function verifyRatingCalculations() {
  ...
  for (const productId of productsWithReviews.slice(0, 5)) { // Check first 5 products
    try {
      ...
      if (product.averageRating === expectedRating && product.numReviews === expectedCount) {
        correctCalculations++;
        console.log(`✅ Product ${productId}: Rating ${product.averageRating} (${product.numReviews} reviews)`);
      } else {
        incorrectCalculations++;
        console.log(`❌ Product ${productId}: Expected ${expectedRating} (${expectedCount}), Got ${product.averageRating} (${product.numReviews})`);
      }
    } catch (error) {
